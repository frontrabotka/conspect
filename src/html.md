...
<span class="cursive"><small class="bold">Дата последней правки: </small><time datetime="2025-07-4"><small>4 июля 2025</small></time></span>

<section>

## <span id="header-1">Отображение сайта. Метаданные</span>

HTML (HyperText Markup Language) — это стандартный язык разметки документов в интернете. Это основа веб-страниц, которая определяет структуру и содержание сайтов.
...
Атрибут `type` в теге `<source>` указывает браузеру формат медиафайла.
...
Атрибут `type` в элементе `<input>` определяет тип поля ввода и его поведение:
+ `text` — простое текстовое поле. Пользователь может ввести любой текст. Это значение *по умолчанию*, если атрибут `type` не указан;
+ `password` — поле для ввода пароля. Вводимые символы отображаются в виде точек или звёздочек для защиты от посторонних глаз;
+ `email` — поле для ввода адреса электронной почты. Браузер проверяет, соответствует ли введённый текст формату электронной почты;
+ `search` — поле для поиска. Выглядит как обычное текстовое поле, но браузеры могут добавлять дополнительные функции, например, кнопку «Очистить» внутри поля;
+ `url` — поле для ввода URL-адреса веб-страницы. Браузер проверяет, соответствует ли введённый текст формату URL;
+ `tel` — поле для ввода номера телефона;
+ `number` — поле для ввода числовых значений;
+ `color` — поле для выбора цвета. Браузер отображает инструмент выбора цвета (обычно палитру или цветовой круг);
+ `date` — поле для выбора даты. Браузер предоставляет календарь для выбора даты;
+ `time` — поле для выбора времени. Браузер предоставляет интерфейс для выбора времени;
+ `datetime-local` — поле для выбора даты и времени (без временной зоны). Браузер предоставляет интерфейсы для выбора даты и времени;
+ `checkbox` — флажок (чекбокс). Используется для выбора одного или нескольких вариантов из списка;
+ `radio` — радиокнопка. Используется для выбора одного варианта из набора. Радиокнопки, принадлежащие к одной группе, должны иметь одинаковый атрибут `name`;
+ `range` — ползунок. Позволяет выбрать числовое значение в заданном диапазоне;
+ `file` — поле для выбора файла. Браузер отображает кнопку «Обзор» или аналогичную, позволяющую пользователю выбрать файл с локального компьютера;
+ `button` — простая кнопка. Не выполняет никаких действий по умолчанию;
+ `submit` — кнопка отправки формы. При нажатии на эту кнопку форма отправляется на сервер;
+ `reset` — кнопка сброса формы. При нажатии на эту кнопку все поля формы возвращаются к своим значениям по умолчанию.
