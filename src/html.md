...
<span class="cursive"><small class="bold">Дата последней правки: </small><time datetime="2025-05-7"><small>7 мая 2025</small></time></span>
...
## <span id="header-8">Подключение файлов</span>
...
Если указать `sizes="any"`, то браузер сможет масштабировать SVG-файл под любой размер, так как SVG по своей природе масштабируется без потери качества.
...
В манифесте должно быть поле `icons`, которое ссылается на три значка: `192x192` для отображения на главном экране, маскируемый значок `512x512` для различных программ запуска Android и значок `512x512`, который будет использоваться как экран-заставка во время загрузки PWA.
