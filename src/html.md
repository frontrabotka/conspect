...
1. Создайте отдельный файл — <span class="blue">vector.svg</span> и внутри пропишите атрибут `xmlns="http://www.w3.org/2000/svg"`, который указывает на используемый диалект XML.
...
Чтобы использовать иконку из спрайта на странице, нужно добавить тег `<svg>` в HTML-разметку и внутрь него тег `<use>` с атрибутом `href`, который ссылается на файл <span class="blue">vector.svg</span> с указанием `id` нужного изображения. Элементы, которые получаются, стилизуются с помощью CSS.
...
Браузеры загружают фавиконки в фоновом режиме, поэтому размер файла не влияет на производительность сайта.
...
Этот метод подключения шрифта через Google Fonts использует параметр `&display=swap`, который автоматически настраивает `font-display: swap`. Это обеспечивает мгновенное отображение текста системным шрифтом, пока загружается веб-шрифт.
...
Атрибут `inputmode="значение"` предоставляет подсказку мобильным браузерам, какую клавиатуру следует отображать для данного поля ввода (`<input>` или `<textarea>`). Браузер может игнорировать этот атрибут, но его использование улучшает пользовательский опыт на мобильных устройствах. Возможные значения:
...
Тег `<details>` создаёт раскрывающийся блок контента («аккордеон»). По умолчанию содержимое блока скрыто, но становится видимым при нажатии на элемент `<summary>`. Внутри `<details>` можно размещать любые элементы, включая теги и интерактивные элементы. Атрибут `open` делает блок открытым по умолчанию при загрузке страницы.
...
Теги `<figure>` и `<figcaption>` используются для связывания медиа-контента (изображения, видео, аудио, код и т.д.) с подписью.
...
+ рекомендуется использовать более двух-трёх слов в имени класса;
