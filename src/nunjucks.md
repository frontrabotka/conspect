...
<span class="cursive"><small class="bold">Дата последней правки: </small><time datetime="2025-07-02"><small>2 июля 2025</small></time></span>
...
✒️ <dfn>Переменные</dfn> в Nunjucks — это именованные контейнеры для хранения значений, которые можно использовать в шаблоне. Переменная ищет значение в контексте шаблона, который обычно содержит данные, переданные из приложения.
...
Переменная создаётся оператором:
...
Переменные доступны только после их объявления и работают внутри текущего шаблона и включённых в него макетов. Имена переменных могут содержать точку (`.`), которая используется для доступа к свойствам объекта: 
...
Если `set` используется на верхнем уровне, то он изменяет значение глобального контекста шаблона. Если `set` используется внутри блоков с областью видимости, таких как `include`, то он изменяет только текущую область видимости.
...
Nunjucks также предоставляет механизм для наследования шаблонов. Блоки, определяемые с помощью тега `{% raw %} {% block %} {% endraw %}`, представляют собой именованные участки в шаблонах, которые можно переопределить в дочерних шаблонах. Как правило, есть открывающая часть `{% raw %} {% block blockName %} {% endraw %}` и закрывающая `{% raw %} {% endblock %} {% endraw %}`.

Чтобы вывести какой-либо из специальных тегов Nunjucks, можно заключить их внутри тега `{% raw %} {% raw %}...{% endraw %} {% endraw %}`. Всё, что находится внутри него, будет выведено как обычный текст. Это полезно, когда нужно показать пример кода Nunjucks на странице без его выполнения:
...
Когда Nunjucks обрабатывает шаблоны, он не удаляет переносы строк и пробелы вокруг тегов. Символом минус (`-`) можно сообщить движку, что нужно удалить все начальные или конечные пробелы. Это полезно для HTML, где лишние пробелы могут влиять на отображение. Вместо `{% raw %} {% {% endraw %}` нужно использовать `{% raw %} {%- {% endraw %}` для удаления пробелов перед тегом Nunjucks, и `{% raw %} -%} {% endraw %}` в конце — для удаления пробелов после тега Nunjucks:
...
То же самое касается переменных: `{% raw %} {{- {% endraw %}` удалит пробелы перед переменной, а `{% raw %} -}} {% endraw %}` удалит пробелы после переменной.
...
✒️ <dfn>Условия</dfn> в Nunjucks — это механизм управления потоком выполнения шаблона, позволяющий отображать разный контент в зависимости от определённых условий. Это позволяет создавать динамические шаблоны, которые адаптируются под различные данные и ситуации.
...
В Nunjucks для организации циклов используется конструкция `for...in`, которая проходит по каждому элементу коллекции:
...
Эта функция помогает поддерживать согласованную структуру на сайте и избегать повторения кода.

Компоненты следуют модели DRY (Don't Repeat Yourself — не повторяйся). Идея состоит в том, что если нужно написать код более одного раза в нескольких местах, то его следует поместить в функцию.
...
Фильтр `reverse` переворачивает строку:
